...
生产文档入库先确定逻辑文档身份，再计算内容版本，最后生成稳定 chunk ID。发布前必须验证完整 snapshot。
...
新 collection 的向量维度、payload index、scope 数量和每个 point hash 全部通过后，才能原子切换 alias。回滚只切回旧 alias，不删除物理 collection。
